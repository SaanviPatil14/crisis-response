...
import { Shield, Bell, HeartPulse, CheckCircle, RefreshCw, Radio, MapPin, User, TriangleAlert, LockKeyhole, Mail, LogOut, Compass, Mic, Map, Menu } from "lucide-react";

// --- THE COMPLEX MATH THING (Haversine Formula) ---
...
export default function StaffPage() {
  ...
  useEffect(() => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => setOfficerLocation({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
        (err) => console.error("Officer GPS Error:", err)
      );
    }
  ...
  const activeCodeBlues = localAlerts.filter(a => a.status === 'CODE_BLUE');
  const pendingAlerts = localAlerts.filter(a => a.status !== 'CODE_BLUE');
  ...
  const handleCodeBlue = async (incidentId) => {
    ...
    // THE SURGICAL FIX: 
    // We record the current timestamp as both 'confirmed_at' (Identified) 
    // and 'dispatched_at' (Help Sent) simultaneously.
    ...
    if (!error && incident) {
      try {
        await fetch('/api/sms', {
          ...
          body: JSON.stringify({ 
            message: `🚨 PUBLIC EMERGENCY DECLARED. Type: ${incident.label}. Location: ${incident.sublabel}. Response Telemetry Logged.` 
          })
        });
      } catch (e) { 
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex font-mono overflow-hidden">
      ...
      {/* LOGIN SCREEN (Centered for both mobile and laptop) */}
      {!session ? (
        ...
        <div className="flex w-full h-screen">
          ...
          {/* LAPTOP: Left Sidebar (Hidden on Mobile) */}
              ...
              {navItems.map(({ id, icon: Icon }) => {
                ...
                return (
                  <button 
                    ...
                    className={`w-full flex items-center gap-4 px-4 py-4 rounded-xl transition-all duration-200 ${isActive ? "bg-sky-500/10 text-sky-400 border border-sky-500/20" : "text-slate-400 hover:bg-slate-800 hover:text-white"}`}
                  >
                    <Icon size={20} className={isActive ? "stroke-[2.5px]" : "stroke-[2px]"} />
                    <span className="font-bold tracking-wide text-sm">{id}</span>
                    {id === "Alerts" && (activeCodeBlues.length > 0 || pendingAlerts.length > 0) && (
          ...
          <main className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 relative">
            ...
            <div className="md:hidden px-5 py-4 bg-slate-900 flex flex-col gap-2 shadow-md z-10 pt-safe">
              <div className="flex items-center justify-between">
                ...
                {(activeCodeBlues.length > 0 || pendingAlerts.length > 0) && <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.6)]" />}
              </div>
            ...
            {/* LAPTOP: Top Header (Hidden on Mobile) */}
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 relative">
              ...
              {activeTab === "Alerts" && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 pb-24 md:pb-8">
                  ...
                  {activeCodeBlues.map(cb => (
                    <div key={cb.id} className="rounded-2xl bg-red-50 border-2 border-red-300 p-6 shadow-sm flex flex-col">
                      <div className="flex items-center gap-2 mb-3"><div className="w-3 h-3 rounded-full bg-red-500 animate-pulse" /><span className="text-red-700 font-black text-sm tracking-widest">CODE BLUE ACTIVE</span></div>
                      <p className="font-bold text-red-900 text-xl mb-2">{cb.label}</p>
                      <p className="text-red-600 text-sm font-medium mb-6 flex-1">{cb.sublabel}</p>
                      <button onClick={() => handleResolve(cb.id)} className="w-full py-3.5 rounded-xl border-2 border-red-200 bg-white text-red-700 font-bold text-sm tracking-wide shadow-sm hover:bg-red-100 transition-colors mt-auto">Stand Down / Resolve</button>
                    </div>
                  ))}
                  ...
                  {activeCodeBlues.length === 0 && pendingAlerts.length === 0 && (
            ...
            {/* MOBILE: Bottom Nav (Hidden on Laptop) */}
